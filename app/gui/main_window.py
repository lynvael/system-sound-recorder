...
LANGUAGE_ITEMS = [
    # Values match app.config.STTSettings.language verbatim (confirmed:
    # "russian" | "english" | "auto"), not ISO codes.
    ("Русский", "russian"),
    ("English", "english"),
    ("Авто", "auto"),
]
...
class MainWindow(QMainWindow):
    ...
    transcript view, and Start/Stop control wired to a background
    `SessionWorker`.
    """
    ...
    def _build_ui(self) -> None:
        ...
        self.language_combo = QComboBox(controls_group)
        ...
        grid.addWidget(self.language_combo, 1, 3)

        self.open_file_button = QPushButton("Открыть аудиофайл…", controls_group)
        self.open_file_button.setEnabled(False)
        grid.addWidget(self.open_file_button, 2, 2, 1, 2)
        ...
        self.open_folder_button = QPushButton("Открыть папку сеанса", central)
        ...
        root.addWidget(self.transcript_view, stretch=3)
        ...
        self._backlog_label = QLabel("Очередь STT: 0")
        for lbl in (self._device_label, self._model_state_label, self._backlog_label):
    ...
    def _update_status_bar(self) -> None:
        ...
        state = "запись" if self._recording else "остановлено"
    ...
    def _on_open_folder_clicked(self) -> None:
        if self._session_dir is None or not self._session_dir.exists():
            self.statusBar().showMessage("Папка сеанса ещё не создана.", 3000)
            return
        QDesktopServices.openUrl(QUrl.fromLocalFile(str(self._session_dir)))
    ...
    def _on_backlog_changed(self, depth: int) -> None:
        self._backlog_label.setText(f"Очередь STT: {depth}")
    ...
    def _on_finished(self, session_dir) -> None:
        ...
        self.start_button.setEnabled(True)
        self.stop_button.setEnabled(False)
        self.open_folder_button.setEnabled(self._session_dir is not None)
