...
Помимо Python-зависимостей, GigaAM требует **ffmpeg в PATH**: его remote-код
декодирует временный WAV через ffmpeg-подпроцесс. На Windows:
`winget install Gyan.FFmpeg` (или https://www.gyan.dev/ffmpeg/builds/), затем
перезапустите приложение. Приложение проверяет наличие ffmpeg при старте
сессии и выдаёт понятную ошибку, если его нет.
...
- **GigaAM — только русский язык** и транскрибирует через временный WAV-файл
  (модель принимает путь к файлу, а не массив в памяти; декодирование WAV идёт
  через ffmpeg в PATH — см. «Установка»).
